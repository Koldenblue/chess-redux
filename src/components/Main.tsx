...
export default function Main() {
  ...
  const [turnDisp, setTurnDisp] = useState(<TurnDisplay>{`White turn`}</TurnDisplay>)
  ...
  const handleSelection = (row: number, col: number): boolean => {
  ...
  const pickEnd = (row: number, col: number, pieceToMove) => {
    ...
    if (validMove) {
      ...
      let newArr: Array<any> = spaceArray.map((x) => {
        return x;
      })
